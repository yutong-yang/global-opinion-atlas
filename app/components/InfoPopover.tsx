...
const encodingGroups = [
  ...
  {
    title: '媒体河流',
    items: ['每条色带是一家媒体（全天发文量 Top 12）：厚度表示该小时发文量，颜色与各视图平台色一致。', '带内圆点为同题转载簇中的报道：颜色代表文章话题，面积对应潜在触达；连线是同一篇通稿的跨媒体转播路径。', '点击点或连线选中通稿（与媒体传播面板联动）；点击色带筛选媒体。'],
  },
  {
    title: '媒体传播',
    items: ['每一行是一组同题报道，每个点是一家转载媒体。', '点面积对应潜在触达，深色描边标记该组最早出现的报道；点颜色代表平台（Top 10 平台全局固定同色，长尾平台为灰色）。', '选中一行通稿后，顶部路径条按发布时刻连线展示其跨平台转播路径；点击路径站点或图例可筛选平台。', '同一平台的颜色在媒体传播、转载网络、媒体构成圈、触达榜与文章列表中保持一致。'],
  },
];
