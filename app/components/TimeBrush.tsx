...
export default function TimeBrush({counts,range,onChange,onInterrupt}:{counts:number[];range:[number,number];onChange:(r:[number,number])=>void;onInterrupt?:()=>void}){
  ...
  const H=52,PAD=4,max=Math.max(...counts,1);
  ...
  return <div ref={ref} className="brush-wrap" onPointerDown={down} role="slider" aria-label="时间刷选">
    <svg width={width} height={H}>
      ...
      {counts.map((c,h)=><rect key={h} x={x(h)-bw*.38} y={H-14-(c/max)*(H-24)} width={bw*.76} height={(c/max)*(H-24)}
        className={inR(h)?'brush-bar':'brush-bar out'}>
    ...
    <span className="brush-label">{`${String(shown[0]).padStart(2,'0')}:00 – ${String(shown[1]).padStart(2,'0')}:59`}</span>
  </div>;
}
